...
def process_user_machine_data(logs, machine_id):
...
@api_view(['GET'])
def user_machine_reports(request, machine_id):
    try:
    ...
    if all_machines:
        ...
        for machine_id in machine_ids:
            ...
            machine_report = process_user_machine_data(machine_logs, machine_id)
        ...
        machine_report = process_user_machine_data(logs, machine_id)
        return Response(machine_report)


@api_view(['GET'])
def all_user_machines_report(request):
    try:
        # Get valid operator IDs from OperatorAFL model
        valid_operators = OperatorAFL.objects.values_list('rfid_card_no', flat=True)
        logs = UserMachineLog.objects.filter(OPERATOR_ID__in=valid_operators)
