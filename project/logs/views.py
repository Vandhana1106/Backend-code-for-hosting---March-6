...
def log_machine_data(request):
    ...
    if not all([machine_id, log_date, start_time, end_time]):
    ...
    if tx_log_id is not None:
        ...
        if tx_log_id > 1000:
            ...
            print(f"Adjusted Tx_LOGID: {adjusted_tx_log_id}")
            ...
            if MachineLog.objects.filter(
                ...
                print("Adjusted Tx_LOGID already exists. Skipping save.")
                return Response({
                    "code": 200,
    ...
    if str_log_id is not None:
        ...
        if str_log_id > 1000:
            ...
            print(f"Adjusted Str_LOGID: {adjusted_str_log_id}")
            ...
            if MachineLog.objects.filter(
                ...
                print("Adjusted Str_LOGID already exists. Skipping save.")
                return Response({
                    "code": 200,
                    "message": "Log saved successfully"
                }, status=200)
    ...
    print("Log saved successfully for MACHINE_ID:", machine_id)
...
def get_underperforming_operators(request):
    ...
    """
    underperforming_modes = [3, 4, 5, 6, 7]  # Non-production modes
...
def operator_reports_by_name(request, operator_name):
    ...
    for mode in mode_hours:
        ...
        elif mode['MODE'] == 6:  # Rework
            total_rework_hours = mode['total_hours'] or 0
        elif mode['MODE'] == 7:  # Needle Break
    ...
    # Calculate non-productive time components (now including rework and needle break)
    ...
    for data in table_data:
        ...
        idle_hours = max(day_total_hours - (
            sewing_hours + 
            meeting_hours + 
            no_feeding_hours + 
            maintenance_hours +
